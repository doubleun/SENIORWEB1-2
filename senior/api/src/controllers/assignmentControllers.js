...
getAbstracts = (req, res) => {
  const abstracts =
    'SELECT subquery.Assignment_ID, subquery.Group_ID,subquery.Major, subquery.Group_Name_Eng,subquery.Group_Name_Thai, subquery.File_Name,subquery.Path,subquery.Submit_Date,subquery.Type,(SELECT GROUP_CONCAT(usr.User_Name) FROM groupmembers gmb INNER JOIN users usr ON gmb.User_Email=usr.User_Email WHERE gmb.Group_ID = subquery.Group_ID AND gmb.Group_Role=0 ) AS Advisor, (SELECT GROUP_CONCAT(usr.User_Name) FROM groupmembers gmb INNER JOIN users usr ON gmb.User_Email=usr.User_Email WHERE gmb.Group_ID = subquery.Group_ID AND ( gmb.Group_Role=2 OR gmb.Group_Role=3) ) AS Students FROM (SELECT gp.Group_ID, gp.Major, gp.Group_Name_Eng,gp.Group_Name_Thai, fl.File_Name,fl.Path,ass.Submit_Date,fl.Type, ass.Assignment_ID FROM files fl INNER JOIN assignments ass ON fl.Assignment_ID=ass.Assignment_ID INNER JOIN groups gp ON ass.Group_ID=gp.Group_ID WHERE fl.Type="Abstract" AND ass.Progress_ID = IF(gp.Is_Re_Eval=0, 8 , 10) AND gp.Grade NOT IN("I","U","F"))AS subquery'
