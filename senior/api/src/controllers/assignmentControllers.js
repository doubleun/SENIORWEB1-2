...
uploadAssignments = async (req, res) => {
  ...
  // Database queries
  const assignmentSql =
    'INSERT IGNORE INTO assignments(Progress_ID, Submit_Date, Group_ID) VALUES (?, ?, ?)'
  const filesSql =
...
updateProgression = (req, res) => {
  ...
  con.query(
    criterias,
    [Major_ID, req.user.projectOnTerm, Progress_ID],
    (err, result, fields) => {
      var giver = 0
      if (err) {
        console.log(err)
...
getAssignment = async (req, res) => {
  ...
  con.query(
    assigment,
    [req.user.projectOnTerm, Group_ID, Progress_ID],
...
getAllAssignment = (req, res) => {
  ...
  con.query(
    sql,
    [req.user.projectOnTerm, Progress_ID],
    (err, result, fields) => {
      if (err) {
        console.log(err)
        res.status(500).send('Internal Server Error')
...
countFileByMajor = async (req, res) => {
  ...
  con.query(
    sql,
    [req.user.projectOnTerm, Major, Progress_ID],
    (err, result, fields) => {
      if (err) {
        console.log(err)
        res.status(500).send('Internal Server Error')
...
getAbstracts = (req, res) => {
  const abstracts =
    'SELECT subquery.Assignment_ID, subquery.Group_ID,subquery.Major, subquery.Group_Name_Eng,subquery.Group_Name_Thai, subquery.File_Name,subquery.Path,subquery.Submit_Date,subquery.Type, (SELECT GROUP_CONCAT(usr.User_Name) FROM groupmembers gmb INNER JOIN users usr ON gmb.User_Email=usr.User_Email AND gmb.Project_on_term_ID=usr.Project_on_term_ID WHERE gmb.Group_ID = subquery.Group_ID AND gmb.Group_Role=0 ) AS Advisor, (SELECT GROUP_CONCAT(usr.User_Name) FROM groupmembers gmb INNER JOIN users usr ON gmb.User_Email=usr.User_Email AND gmb.Project_on_term_ID=usr.Project_on_term_ID  WHERE gmb.Group_ID = subquery.Group_ID AND ( gmb.Group_Role=2 OR gmb.Group_Role=3) ) AS Students FROM (SELECT gp.Group_ID, gp.Major, gp.Group_Name_Eng,gp.Group_Name_Thai, fl.File_Name,fl.Path,ass.Submit_Date,fl.Type, ass.Assignment_ID FROM files fl INNER JOIN assignments ass ON fl.Assignment_ID=ass.Assignment_ID INNER JOIN groups gp ON ass.Group_ID=gp.Group_ID WHERE fl.Type="Abstract" AND ass.Progress_ID = IF(gp.Is_Re_Eval=0, 8 , 10) AND gp.Grade NOT IN("I","U","F"))AS subquery'
